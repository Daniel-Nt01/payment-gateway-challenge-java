...
- **`getExpiryDate()` removed from `PostPaymentRequest`** - this method formatted the expiry date for the bank simulator request, which is not the responsibility of a request DTO. It was moved to the `CreateExpiryDate` stateless utility class as a static method currently to be used in `BankRequestMapperImpl` as a concern of the bank request mapping as well as to test in isolation.
